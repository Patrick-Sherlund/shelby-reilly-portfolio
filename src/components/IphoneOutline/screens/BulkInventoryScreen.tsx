...
interface Item { id: number; name: string; qty: number }
          ...
          <React.Fragment key={i.id}>
            <ListItemButton sx={{ px: 0 }} onClick={() => alert(i.name)}>
              <ListItemText
                ...
                secondary={`item #: ${i.id}`}
                primaryTypographyProps={{ fontFamily: FONT_FAMILY, fontSize: 10, fontWeight: 700, color: FONT_COLOR }}
                secondaryTypographyProps={{ fontFamily: FONT_FAMILY, fontSize: 8, color: FONT_COLOR }}
              />
              <Typography sx={{ fontFamily: FONT_FAMILY, fontSize: 10, fontWeight: 700, color: FONT_COLOR }}>
                QTY: {i.qty}
              </Typography>
            </ListItemButton>
