    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
        <IconButton size="small" sx={{ p: 0 }}>
          {/* Using the same icon as design shows an "X" */}
      ...
      <Box sx={{ flex: 1, overflow: 'auto' }}>
        {lots.map((l) => (
          ...
          >
            {/* Header row */}
            <Box sx={{ display: 'flex', alignItems: 'center', cursor:'pointer' }} onClick={() => toggleExpand(l.id)}>
              <Typography
                sx={{ fontFamily: FONT_FAMILY, fontWeight: 700, fontSize: 10, color: FONT_COLOR, flex: 1 }}
              >
                {l.name}
              </Typography>
              <ExpandMoreIcon sx={{ fontSize: 16, color: FONT_COLOR, transform: expanded.has(l.id)?'rotate(180deg)':'none', transition:'transform 0.2s' }} />
            </Box>
            {expanded.has(l.id) && (
            <>
            ...
            <TextField
              ...
              size="small"
              type="number"
              inputProps={{ min: 0, max: l.total, style: { border:'1px solid #D6D6D6', borderRadius: 2, textAlign: 'left' } }}
              sx={{
                width: '100%',
                '& .MuiInputBase-input': {
                  fontFamily: FONT_FAMILY,
            ...
            />
            <Typography sx={{fontFamily: FONT_FAMILY, fontSize: 8, color: FONT_COLOR, mt: 0.5, textAlign:'right' }}>
